...
#ikeaload148
k4 >> ikea(dur=8, valad=1200, valadr=0.1, valadd=10, valadt=1, valadc=0.3).unison(4)
k5 >> ikea(dur=8, leg=12, hhat=0.5, sn=0.7, harm=0, bell=0.5, bellf=1000, bellq=0.9, dafilter=1200, dastart=250, darel=0.2, darq=0.5, datype=0, pshift=4, mverb=0.5)
k6 >> ikea(shift=4, dur=16, formant=1).unison(2)
k4.dur=4
k4.sn=1

a1 >> loop("rvoice16", dur=16, sample=1, spf=(100, PWhite(2000, 500)), spfend=(PWhite(200, 5000), 500), chop=4, shift=(P*[1, 2, 0.5], P*[1, 2, 0.5]), amp=4, chopmix=(0.5, 0.25, 2), feed=0.5, delay=(0, 4, 8), spfslide=16, hpf=800).unison(2)
a3 >> loop("rbot4", dur=16, sample=3, shift=1, hpf=1000)
l0 >> loop("cyber32", dur=32, sample=8)
a2 >> organ([3, (3, [4, var([3, 14], [15, 1])])], slide=0.5,dur=[2, 1/2, 1, 1/2], oct=(7, var([5, 6], [24, 8]), linvar([5.99, 6.01])), scale=Scale.chromatic, shape=0, lpf=0, bpf=linvar([2000, 6200, 32], [16, 8, 32]), shape=0, high=0, mid=0.1, low=0.1, vol=0.8, chop=0).unison(8).spread(2)
k5 >> play("- ")
k4 >> play("[o.o]", valad=500, valadr=0.3, valadd=5, valadt=0, valadc=0.3)
a1 >> sawbass([3, (3, [4, 3])], dur=1/2, oct=(4, var([5, 6], [24, 8]), linvar([5.99, 6.01])), scale=Scale.chromatic, shape=[0.5, 0.1], lpf=1600, bpf=linvar([200, 1600, 32], [16, 8, 32]), shape=PTri(0, 1), high=1, mid=4, low=0.1, vol=0.6).unison(2)

g1 >> play(PRand("Xx.x{Cxxx}.x{c--}.x-.x[xxx].x"))
...
g4 >> play("-", sample=4, rate=linvar([PRand([1, 2, 4, 8, 16]), 1], PRand([1, 2, 4, 8])), dur=var([PRand([4, 16, 32]), 1/4]), shift=PRand([2, 4, 8]))
...
a1 >> sawbass([3, (3, [4, 3])], dur=1/2, oct=(4, 3, 5), scale=Scale.chromatic, shape=[0.5, 0.1], lpf=1600, bpf=linvar([200, 1600, 32], [16, 8, 32]), shape=PTri(0, 1), high=1, mid=1, low=0.1, vol=0.8).unison(8)
...
e1 >> play("x.........-.....", dur=0.125, sample=(3,5), amp=1, shape=PCoin(0, 1, 0.1)).every(1, "stutter", 1, rate=2, shape=4)
...
r_all.degree=([4, 2, 0])
d1 >> play("W ", shape=0, dur=4, bpf=80, bpr=0.9, amp=PMorse("thisiskickistooloud"),vol=0.5, slide=[0, -4], rate=var([1, linvar([1, 0.2], 4)])).unison(4)
d8 >> play("@ ", dur=PDur(1, 15, 4), echo=0.25)
d2 >> brown(dur=8, cut=1/2, room2=0.2, chop=4, damp2=0.2, fold=0.5, lofi=0.5, hpf=4000)
r1 >> ssaw([3, (3, [4, 3])], scale=Scale.chromatic, dur=1/4, amp=2, crush=8, room2=1, mix2=0.2, bits=8, fmod=4, lpf=0).slider()
...
a1 >> sawbass([3, (3, [4, var([3, 14], [15, 1])])], slide=a2.degree == 15,dur=[1/2, 1/2, 1], oct=(4, var([5, 6], [24, 8]), linvar([5.99, 6.01])), scale=Scale.chromatic, shape=var([[0.5, 0.1], [4, 5]], [48, 16]), lpf=1200, bpf=linvar([200, 3200, 32], [16, 8, 32]), shape=PTri(0, 1), high=1, mid=8, low=0.1, vol=0.5, chop=0).unison(8)
g_all.stop()
...
Master().cut=0
a1 >> sawbass([3, (3, [4, 3])], dur=[1/2, 1/2, 1], oct=(4, var([5, 6], [24, 8]), linvar([5.99, 6.01])), scale=Scale.chromatic, shape=[0.5, 0.1], lpf=1600, bpf=linvar([200, 1600, 32], [16, 8, 32]), shape=PTri(0, 1), high=1, mid=4, low=0.1, vol=0.8).unison(8)
...
# [noloop not registered] l2 >> noloop("vocalcrash8", dur=[16, 8, 4, 2], lofi=1, start=0.5, rate=1, sample=4)
...
l2 >> loop("noizebeat16", lpf=0, amp=1)
