...
g2 >> dbass(dur=PDur(9, 12), drive=PStep(9, 1,0), tube=1, fbdelay=0.2, tape=1, tubewarmth=20, leg=0, rate=linvar([2, .1],[64,0]), glide=0.01, lpf=PFr(500,2200)).unison(2)
g3 >> play("[--]", sample=5, hpf=12000, pan=PWhite(-1,1)).human(60, 10)
g4 >> loop("noizebeat16", dur=8, sample=5, amp=[1, 1, 0, 1]).lclip(PRand([4, 0.5, 1, 2, 4]))
g2 >> dbass(dur=PDur(11, 12), drive=PStep(9, 1,0), tube=2, fbdelay=0.2, tape=1, tubewarmth=20, leg=0, rate=linvar([4, .1],[64,0]), glide=0.01, lpf=PFr(500,2200)).unison(2)
...
g2 >> cbass(dur=PDur(11, 16), follow=4, drive=PStep(9, 1,0), tube=2, fbdelay=0.1, tape=1, dist2=0.3, tubewarmth=20, leg=0, rate=linvar([4, .1],[64,0]), glide=0.00, lpf=PFr(500,2200)).unison(2)
g2 >> cbass(dur=4)
g2.bitrot=0.3
g2.drift=0
g8 >> play(PRand("Xx.xx.xx.xx.xx.x"), room2=0, amp=4, tape=1, tapedrive=2, mix2=0, revatk=0, revsus=0, sample=[0,7,4], lpf=0, leg=PRand(0,42), krush=P*[0,4]).sometimes("stutter", PRand(8), rate=PRand(8)).slider(on=0)
