...
fx = FxList.new('sbrk', 'stutbreak', {'sbrk': 0.5, 't_reset': 0, 'sbrkdur': 0.5, 'sbrkmix': 1.0, 'sus': 1}, order=2)
fx.add_var("dry")
fx.add_var("reset")
fx.add_var("wet")
fx.add_var("stuti")
...
fx = FxList.new("wshape", "waveshaper", {"wshape": 0, "wgain": 1, "wmix": 0.5}, order=2)
...
fx = FxList.new("fuzz", "fuzz", {"fuzz": 0, "fuzzgain": 2, "fuzztone": 0.5, "fuzzoctave": 0.3}, order=2)
...
fx = FxList.new("tape", "tape", {"tape": 0, "tapedrive": 1.5, "tapewarm": 0.5, "tapewobble": 0.1}, order=2)
...
fx = FxList.new("envdist", "envdist", {"envdist": 0, "envdistgain": 2, "envdistsens": 1, "envdistattack": 0.01}, order=2)
...
fx = FxList.new("tube", "tubedrive", {"tube": 0, "tubegain": 1.5, "tubewarm": 0.6, "tubebias": 0.1}, order=2)
...
fx = FxList.new("pumper", "pumper", {"pumper": 0, "pumprate": 4, "pumpattack": 0.005, "pumprel": 0.2, "pumpcurve": -4, "beat_dur": 1}, order=2)
fx.doc("Sidechain simulation - creates pumping effect synced to beat")
fx.add_var("dry")
fx.add_var("pumpEnv")
fx.add_var("rate")
fx.add("dry = osc")
fx.add("rate = pumprate / beat_dur")
...
fx = FxList.new("transient", "transient", {"transient": 0, "transattack": 1, "transsustain": 1, "transtime": 0.02}, order=2)
...
fx = FxList.new("multicrush", "multicrush", {"multicrush": 0, "mclowdrive": 1.5, "mcmiddrive": 2, "mchighdrive": 1.8, "mclofreq": 200, "mchifreq": 3000}, order=2)
...
fx = FxList.new("combres", "combres", {"combres": 0, "combfreq": 200, "combdecay": 0.3, "combmix": 0.5, "combspread": 0.01}, order=2)
...
fx.add("osc = XFade2.ar(dry, dry + (resonated * combmix), combres * 2 - 1)")
fx.save()

# Rhythmic gate - trance gate synced to beat
fx = FxList.new("gate", "rhythmgate", {"gate": 0, "gaterate": 4, "gatewidth": 0.5, "gatesmooth": 0.01, "gatewave": 0, "beat_dur": 1}, order=2)
fx.doc("Rhythmic gate - choppy textures synced to beat (wave: 0=square, 1=saw, 2=sine)")
fx.add_var("dry")
fx.add_var("gateEnv")
fx.add_var("rate")
fx.add_var("smoothGate")
fx.add("dry = osc")
fx.add("rate = gaterate / beat_dur")
fx.add("gateEnv = Select.kr(gatewave, [LFPulse.kr(rate, width: gatewidth), LFSaw.kr(rate).range(0, 1), SinOsc.kr(rate).range(0, 1)])")
fx.add("smoothGate = Lag.kr(gateEnv, gatesmooth)")
fx.add("osc = XFade2.ar(dry, osc * smoothGate, gate * 2 - 1)")
fx.save()

# Stereo width - mono bass, wide highs
fx = FxList.new("stereowidth", "stereowidth", {"stereowidth": 0, "swfreq": 300, "swnarrow": 0, "swwide": 1.5}, order=2)
...
fx = FxList.new("bitrot", "bitrot", {"bitrot": 0, "rotbits": 8, "rotrate": 0.5, "rotjitter": 0.1}, order=2)
...
fx = FxList.new("fbdelay", "fbdelay", {"fbdelay": 0, "fbtime": 0.3, "fbfeed": 0.7, "fbcutoff": 3000, "fbspread": 0.02, "beat_dur": 1}, order=2)
...
fx = FxList.new("freeze", "spectralfreeze", {"freeze": 0, "freezemix": 0.5, "freezerand": 0}, order=2)
fx.doc("Spectral freeze - captures and holds spectrum for drones")
fx.add_var("dry")
fx.add_var("chain")
fx.add_var("frozen")
fx.add("dry = osc")
fx.add("chain = FFT(LocalBuf(2048), osc)")
...
fx = FxList.new("decimate", "decimate", {"decimate": 0, "decbits": 4, "decrate": 4000, "decsmooth": 0}, order=2)
...
fx = FxList.new("glitch", "glitch", {"glitch": 0, "glitchrate": 8, "glitchdepth": 0.5, "glitchcrush": 0.3, "glitchchance": 0.5, "beat_dur": 1}, order=2)
...
fx = FxList.new("tstop", "tapestop", {"tstop": 0, "tstoptime": 0.5, "tstopcurve": -4, "tstopdir": 0, "sus": 1}, order=2)
...
fx = FxList.new("drift", "drift", {"drift": 0, "driftspeed": 2, "driftdepth": 0.02, "driftsmooth": 0.5}, order=2)
