...
with SynthDef("noise") as noise:
    ...
    noise.osc = LFNoise0.ar(noise.freq, noise.amp*0.7)
    noise.env = Env.perc()

with SynthDef("dab") as synth:
     synth.amp = synth.amp * 0.6
...
with SynthDef("bass") as bass:
    ...
    bass.freq = bass.freq / 4
    bass.amp  = bass.amp * 0.45
    bass.osc  = LFTri.ar(bass.freq, mul=bass.amp) + VarSaw.ar(bass.freq, width=bass.rate / 10, mul=bass.amp) + SinOscFB.ar(bass.freq, mul=bass.amp / 2)
    bass.env  = Env.perc(atk=bass.atk, curve="'lin'", )

with SynthDef("dirt") as dirt:
    dirt.freq = dirt.freq / 4
    dirt.amp  = dirt.amp / 2
    dirt.osc  = LFSaw.ar(dirt.freq, mul=dirt.amp) + VarSaw.ar(dirt.freq + 1, width=0.85 + dirt.vib, mul=dirt.amp) + SinOscFB.ar(dirt.freq - 1, mul=dirt.amp/2)
    dirt.env  = Env.perc()

with SynthDef("crunch") as crunch:
    crunch.amp = crunch.amp * 2
...
with SynthDef("rave") as rave:
    rave.osc = Gendy1.ar(rave.rate-1, mul=rave.amp*0.7, minfreq=rave.freq, maxfreq=rave.freq*2)
    rave.env = Env.perc()

with SynthDef("scatter") as scatter:
    scatter.osc = (Saw.ar( scatter.freq , mul=scatter.amp / 2) + VarSaw.ar(scatter.freq + [2,1],  mul=scatter.amp/2)) * LFNoise0.ar(scatter.rate)
...
with SynthDef("dub") as dub:
    dub.defaults.update(rate=1)
    dub.freq = dub.freq / 4
    dub.amp  = dub.amp * 0.4
    dub.osc  = LFTri.ar(dub.freq, mul=dub.amp) + SinOscFB.ar(dub.freq, feedback=dub.rate,mul=dub.amp)
    dub.env  = Env.sine(dur=dub.sus)

with SynthDef("viola") as viola:
    viola.defaults.update(vib=6)
    viola.amp = viola.amp/1.2
...
with SynthDef("feel") as feel:
    ...
    feel.osc = feel.osc * feel.amp
    feel.adsr(atk=0.1, decay=1, rel=0.01)

with SynthDef("glass") as glass:
    glass.sus = glass.sus * 1.5
    glass.amp = glass.amp * 5
...
with SynthDef("soft") as soft:
    ...
    soft.amp = soft.amp / (100 * (1 + soft.rate))
...
with SynthDef("creep") as creep:
    creep.defaults.update(rate=1)
    creep.amp = creep.amp / 2
    creep.osc = PMOsc.ar(creep.freq, creep.freq * creep.rate, 10)
    creep.env = Env.reverse()

# No context manager SynthDef creation

orient = SynthDef("orient")
orient.defaults.update(room=0.7, mix=0.3)
orient.amp = orient.amp * 1.5
...
pulse.defaults.update(rate=0.5)
pulse.amp = pulse.amp / 4
pulse.osc = LFPulse.ar(pulse.freq, pulse.rate)
pulse.osc = pulse.osc * pulse.amp
pulse.env = Env.mask()
pulse.add()

saw = SynthDef("saw")
saw.defaults.update(rate=0)
saw.amp = saw.amp / 4
saw.osc = LFSaw.ar(saw.freq, saw.rate)
saw.osc = saw.osc * saw.amp
saw.env = Env.mask()
saw.add()

snick = SynthDef("snick")
snick.amp = snick.amp*4
...
arpy.osc  = LPF.ar(Impulse.ar(arpy.freq, arpy.rate), 5000)
arpy.env  = Env.perc(sus=Env.sus * 0.5)
arpy.add()

nylon = SynthDef("nylon")
nylon.osc = LFPulse.ar(nylon.freq, 0.5, 0.33 * nylon.rate, 0.25) + LFPar.ar(nylon.freq + 0.5, 1, 0.1, 0.25)
nylon.env = Env.perc(amp=Env.amp*0.7, curve=-4, atk=0.000125, sus=Env.sus * 3)
nylon.add()
...
with SynthDef("jbass") as jbass:
    jbass.freq = jbass.freq / 3
    jbass.amp  = jbass.amp * 0.4
    jbass.osc  = LFTri.ar(jbass.freq, iphase= jbass.rate, mul=jbass.amp)
