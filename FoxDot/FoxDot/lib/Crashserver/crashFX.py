...
fx = FxList.new("wshape", "waveshaper", {"wshape": 0, "wtype": 0, "wmix": 0.5}, order=2)
...
fx.add_var("wt10")
fx.add_var("wt11")
fx.add("input_signal = (osc * wshape).clip(-1, 1)")
fx.add("x = input_signal")
fx.add("wt0 = (x.sign * ((x.abs ** 0.2) - (x.abs ** 2.5)))*0.5")
fx.add("wt1 = ((x * 4).fold(-1, 1) * 1.3 + (x * 1.7))")
fx.add("wt2 = (x.sign * ((x.abs ** 0.4) * (1 - (x.abs ** 0.6))) + (x * 0.1))")
fx.add("wt3 = x.sign * (sin(x.abs * pi * 2) * x.abs * 2)")
fx.add("wt4 = x + ((x * 8).sin * 0.4 * exp(-3 * x.abs))")
fx.add("wt5 = (x.sign * (x.abs ** (1 + x.abs * 0.4)) * (1 - (x.abs * 0.1)))")
fx.add("wt6 = (x.sign * (x.abs ** (3 - 2.5 * x.abs)))*0.3")
fx.add("wt7 = (x.tanh + (x ** 3 * 0.2 * (1 - x.abs)) + ((x * 3).sin * 0.05))*2")
fx.add("wt8 = (x * 6).sin * x.abs ** 0.3 + (x * 0.4)")
fx.add("wt9 = Select.ar(x.abs > 0.5, [x ** 0.4, x.sign * (2 - (2 - x.abs))])")
fx.add("wt10 = (x * 3).wrap(-0.5, 0.5) * 2.5 + (x * 0.3)")
fx.add("wt11 = x.sign * (x.abs ** (0.5 + x.abs)) * (1 - x.abs ** 2)")
fx.add("shaped = Select.ar(wtype, [wt0, wt1, wt2, wt3, wt4, wt5, wt6, wt7, wt8, wt9, wt10, wt11])")
fx.add("shaped = LeakDC.ar(shaped)")
