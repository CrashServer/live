...
# ZERFALL
# Violent Syncopated Industrial
# CRASH Server // TOPLAP Strasbourg
x8 >> a_bassry(distortion=10, cutoff=700, resonance=0.6, pattern=0, swing=0.1)
Root.default = "A"
Scale.default = "minor"
...
a1 >> fbass(a0.degree * var([1, -1], 8) + var([0, 7, 12], [8, 4, 4]), dur=PDur(var([7, 5, 9], [4, 4, 8]), 16), oct=PStep(6, (3, 4), 5), lpf=expvar([600, 8000], 16), fmod=linvar([4, 28], [32, 0]), sus=var([1/8, 1/4, 1/16], [6, 4, 6]), shape=linvar([0.2, 0.9], 48), amp=1).unison(4)
b1 >> click(var([0, a0.degree, 7], [4, 10, 2]), dur=PDur(var([3, 5, 7], PRand(8, 16)), 16), oct=(PStep(5, 6, 4), 5), shape=var([0.3, PWhite(0.6, 0.95)], [6, 2]), triode=linvar([2, 8], 32), octer=var([0, 1], [14, 2]), octersub=3, hpf=linvar([200, 1200], 48), amp=0.4, mverb=PStep(12, 0.6, 0)).unison(2).sometimes("stutter", PRand(4, 8), pan=[-1, 1])
...
Scale.default = "minor"
# d1 >> dbass(dur=1/2)
b2 >> pumpbass([0], dur=0.5, cutoff=linvar([400,1200],8), pumprate=4, pumpdepth=0.6, growl=linvar([0.1, 4],32), amp=0.55, oct=4, gate=0.5, drift=0, fshift=0)
q1 >> compperc(dur=PDur(5,16), decay=linvar([0.1, 0.7], 128), metal=linvar([0.2, 12], [32, 2]), amp=1, pan=PWhite(-0.4,0.4), oct=4, tremolo=0, beat_dur=1, tremolomix=0, mverb=0.2, leg=4)
sb >> hardstab([([0, 0, 0, 4, 1],4,7)], dur=4, fbdelay=PWhite(0.1, 0.9), fbtime=0.75, fbfeed=0.3, fbcutoff=2000, amp=0.2, gate=0.6)
d1 >> sawbass(var([0, a0.degree, -5], [12, 4, 16]), gate=0.1, fbdelay=0.5, dur=[8, 1, 1, 2, 2, 2], oct=6, lpf=linvar([80, 400], 64), shape=linvar([0.15, 0.5], 32), amp=0.75, sus=var([4, 2], [8, 8])).unison(2, 0.02, 20)
d2 >> cbass(0, dur=1/4, oct=6, follow=4, amp=[0, 0.5, 0.3, 0.5] * var([1, 0], [34, 2]), lpf=1200, pumper=4, transient=0.3, shape=1, rate=0.5)
p1 >> compperc(dur=PDur(5,16), freq=P[900,1100,1000], metal=4, pumper=1, amp=2, dist2=4.5)
sb.dur=8
h1 >> play("-", amp=var([[0.3, 0.3, 0.9, 0.4], [0.5, 0.5, 1, 0.5], [0.4, 0, 0.8, 0.5]], [6, 6, 4]), dur=1/4, sample=var([3, 2, 8], 16), hpf=linvar([4500, 8000], 24), lpf=expvar([7000, 18000], 16), rate=var([1, PWhite(0.85, 1.3)], [12, 4]), pan=PWhite(-0.4, 0.4))
pd >> darkpad([([1, 0, 4],4,7)], dur=8, pumper=1.1, pumprate=1, pumprel=0.25, amp=0.3)
h2 >> pumphihat(dur=0.25, amp=P[0.4,0.15,0.3,0.15], decay=0.04, metal=0.6, open=P[0,0,0.2,0], amplify=4)
pd >> darkpad([(4,4,7)], dur=8, pumper=1.1, pumprate=1, pumprel=0.25, amp=0.5)
sb.dur=4
...
sb.stop()
h1.stop()
p1 >> prophet([0,2,4,5,7,5,4,2], dur=0.5, cutoff=linvar([2000,6000],16), rq=0.4, rate=0.5, phase=0.5, mverb=0.4, amp=0.05)
...
#main
dl >> darklead(P[0,2,4,7,9,7,4,2], dur=0.25, cutoff=linvar([1500,5000],8), detune=0.01, shape=1.5, sub=0.2, fbdelay=0.3, fbtime=0.5, fbfeed=0.35, oct=4, amp=0.125).sometimes("degree.add", 7)
q3 >> compkick(punch=0.5, comp=16, release=0.35, click=1.0,shape=0.4, sub=1, tape=1, tapedrive=2, body=1.2, tone=0.0, oct=3, echo=0)
# p1 >> compperc(dur=PDur(5,16), freq=P[900,1100,1000], decay=0.06, metal=0.6, ring=-0.6, comp=10, amp=1, pan=PWhite(-0.5,0.5))
b2.stop()
q1.stop()
d2.stop()
h1.stop()
...
a1 >> acidline([0,0,3,0,5,0,0,3], dur=0.25, cutoff=linvar([400,2000],8), res=linvar([0.5,0.8],4), shape=2.5, accent=P[0,0,1,0]*0.5, tubedrive=0.4, amp=2)
a1 >> acidline(var([seq], [16, 8, 8]), dur=var([0.25, 0.125, 0.25], [24, 4, 4]), cutoff=expvar([300, var([2000, 4000], [16, 16])], 8), res=linvar([0.4, 0.9], 16), shape=linvar([1.5, 4], 32), accent=var([P[0,0,1,0]*0.5, P[1,0,0,1]*0.6, P[0,1,0,1]*0.4], [12, 8, 12]), tubedrive=linvar([0.2, 0.7], 48), amp=var([0.6, 0.8, 0.5], [12, 4, 16]))
...
d2 >> play("-", sample=6, dur=0.25, amp=P[0.4,0.15,0.3,0.15])
...
hv >> hoover([(0,4,7)], dur=16, porta=1.5, portadur=0.2, tubedrive=0.5, tubewarm=0.4, amp=0.8, sus=6)

sn >> industrialsnare(dur=2, rattle=0.6, snap=0.7, shape=4, comp=12, amp=0.9)
b2 >> pumpbass([0,0,0,2], dur=0.5, cutoff=linvar([400,1200],16), pumprate=4, pumpdepth=0.5, growl=0.3, amp=1.5, hpf=200)
k4 >> compkick(dur=1, punch=0.8, click=0.5, comp=12, shape=4, amp=0.9, oct=3, tape=4, tapedrive=1)
...
dl >> darklead(melody(), dur=0.5, cutoff=linvar([1000,4000],8), detune=0.006, sub=0.3, rhythmgate=var([0,0.5],[8,8]), gaterate=4, oct=5, amp=0.1)
...
d1 >> play("X.", sample=4, amp=1.9, dur=0.5)

dp.stop()
hw.stop()
pr.stop()
# d1 >> play("<X.><X[.X]><X.><[XX].>", sample=8, dur=0.5, amp=0.9)
d2 >> play("-", sample=5, dur=0.25, amp=P[0.4,0.15,0.3,0.15], hpf=4000)
d3 >> play("..C.", sample=2, dur=0.5, amp=0.55)

a1 >> acidline([0,0,3,0,5,0,0,3], dur=0.25, cutoff=linvar([400,2000],8), res=linvar([0.5,0.8],4), shape=2.5, accent=P[0,0,1,0]*0.5, tubedrive=0.4, amp=2, mverb=0.5, gate=0.8)
a1 >> acidline(var([seq], [16, 8, 8]), dur=var([0.25, 0.125, 0.25], [24, 4, 4]), cutoff=expvar([300, var([2000, 4000], [16, 16])], 8), res=linvar([0.4, 0.9], 16), shape=linvar([1.5, 4], 32), accent=var([P[0,0,1,0]*0.5, P[1,0,0,1]*0.6, P[0,1,0,1]*0.4], [12, 8, 12]), tubedrive=linvar([0.2, 0.7], 48), amp=var([0.6, 0.8, 0.5], [12, 4, 16]), mverb=0.5, oct=4)
...
hv.stop()
a1 >> acidline([0,0,3,0,5,0,0,3], dur=0.25, cutoff=linvar([400,2000],8), res=linvar([0.5,0.8],4), shape=2.5, accent=P[0,0,1,0]*0.5, tubedrive=0.4, amp=4)
d3 >> play("<....><..u.><....><.u.u>", sample=3, amp=0.55)
# d1 >> play("[XX].X..", dur=0.25, sample=4, amp=0.9)
d5 >> play("<+...><..+.><....><.+..>", dur=0.25, sample=2, amp=0.25)
l4 >> loop("metaldrum32", dur=32, amp=4)
...
d4 >> play("-", dur=0.25, sample=2, amp=P[0.35,0.15,0.25,0.15], hpf=6000)
...
# SCHMELZWERK
# Evolving Industrial Architecture
# CRASH Server // TOPLAP Strasbourg
...
# === CORE SEQUENCE - everything derives from this ===

# k1 >> compkick(dur=var([1, 1, P[1,1,1,1/2,1/2]], [16, 8, 8]), punch=linvar([0.5, 0.95], 48), click=var([0.4, 0.7, 0.5], [8, 4, 4]), comp=var([8, 14], [24, 8]), shape=linvar([2, 6], 64), amp=0.85, oct=var([3, 3, 2], [14, 2, 16]))
