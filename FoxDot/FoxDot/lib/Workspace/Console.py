...
class Counter(Canvas):
	...
	def redraw(self):
		...
		beat8 = int((self.metro.now() - self.metro.get_latency()) % 8)
		beat16 = int((self.metro.now() - self.metro.get_latency()) % 16)
		beat32 = int((self.metro.now() - self.metro.get_latency()) % 32)
		beat64 = int((self.metro.now() - self.metro.get_latency()) % 64)
		beat128 = int((self.metro.now() - self.metro.get_latency()) % 128)
		...
		self.create_text((w - width)*1/6, h / 2,
							 justify=RIGHT,
							 text=f"{beat8 + 1}/8",
							 font=self.font,
							 fill="#c9c9c9")

		self.create_text((w - width)*2/6, h / 2,
							 justify=RIGHT,
							 text=f"{beat16 + 1}/16",
							 font=self.font,
							 fill="#c9c9c9")

		self.create_text((w - width)*3/6, h / 2,
							 justify=RIGHT,
							 text=f"{beat32 + 1}/32",
							 font=self.font,
							 fill="#c9c9c9")

		self.create_text((w - width)*4/6, h / 2,
							 justify=RIGHT,
							 text=f"{beat64 + 1}/64",
							 font=self.font,
							 fill="#c9c9c9")

		self.create_text((w - width)*5/6, h / 2,
							 justify=RIGHT,
							 text=f"{beat128 + 1}/128",
							 font=self.font,
							 fill="#c9c9c9")
