...
from ..Patterns.Sequences import PRange, P, Pattern
import math
...
def exoScale(name="", tuni = 99):
	if (name == ""):
		print([cls.__name__ for cls in Tunings.__subclasses__()])
	else:
		print(name.names())
		print(f"ex : Scale.default.set({str(name.__name__)}.{name.names()[0]},tuning={str(name.__name__)}.tuning)")
