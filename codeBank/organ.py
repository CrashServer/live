...
~r1 >> organ(P[var([0,-2,-3,-4],8),2,PStep(8,5,4),2], dur=var([PDur(4,8),PDur([5,3,6],8)]), amp=1, crush=1, crush_=4, crush_d=2, bits=6, bits_=var([12, 16]), fmod=1, lpf=2800, lpr=0.4).solo(1)
...
b8 >> lbass(var([0, -2, linvar([-2,0],[8,0])], [16,8,8]), dur=var([1/2,1/4],[24,8]), cutoff=PFr(4000,12000), detune=0.9, tone=PFr(0.16,0.9), dist2=0.9, oct=var([5,lininf(5,6,8)],[24,8]))
...
p1 >> play("<E ><p >", sample=0, dur=PDur([3, 5], 8), amp=PWhite(2,1), hpf=220, chop=1/2, leg=PWhite(15), pan=PWhite(-1,1), fx2=1)
...
k4 >> lapin(([-4, -3], 4), oct=(3, 4), shape=0.4, fdistfreq=4000, dur=8, lpf=4000, fdist=1, hpf=200, bpf=0, lofi=1, amp=PCoin(0.2, 0, 0.25)).unison(2).slider().solo(0)
...
j1 >> bell(P[var([0,-2,-3,-4],8),2,PStep(8,5,4),2],dur=4, amp=1, feed=0.5, oct=(3, 4), mverb=0.5)

j2 >> lbass(P[var([0,-2,-3,-4],8),2,PStep(8,5,4),2],dur=2, amp=1, feed=0.5, oct=(3, 4, 5), mverb=0.8)
